...
The world is not a barrier to G-dliness; it is the very material from which we construct a home for the Divine.

In Chassidus, we learn that there is no place void of Him. This means that the studio, the runway, the boardroom, and the gym are not "secular" spaces—they are simply unrefined. They are waiting for a King.
...
const HARDCODED_UPDATES = [
    { title: 'Mincha Schedule Change', body: 'Starting Sunday, Mincha will be at 4:30 PM due to early sunset. Please check the new winter schedule.', type: 'ALERT' },
    { title: 'Tonight: Farbrengen', body: 'Join us at the Main Shul for a Yud Tes Kislev gathering. 8:00 PM. Guest speaker Rabbi Jacobson.', type: 'EVENT' },
    { title: 'Job Fair Next Week', body: 'The annual community Parnassah Expo is happening Tuesday at the Event Hall. Bring your resumes.', type: 'INFO' },
    { title: 'New Mikvah Hours', body: 'Women\'s Mikvah is now open until 11:00 PM on weeknights.', type: 'INFO' }
];
    ...
    <div className="relative min-h-full">
        ...
        <section className="animate-slide-up">
            <div className="flex justify-between items-center mb-6 px-2 border-b border-stone-100 pb-4">
                ...
                <button onClick={() => onNavigate('habits')} className="text-xs font-bold text-king-primary uppercase tracking-wider hover:underline active:opacity-70">View All</button>
            </div>
                    ...
                    return (
                        ...
                        >
                            <div className={`w-8 h-8 shrink-0 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${
                                isCompleted ? 'bg-king-primary border-king-primary text-white scale-110' : 'border-stone-200 text-transparent group-hover:border-king-primary/50'
                            }`}>
        ...
        <section className="animate-slide-up delay-400">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {HARDCODED_UPDATES.map((update, idx) => (
                    <div key={idx} className={`p-6 rounded-3xl border flex gap-4 transition-all hover:-translate-y-1 hover:shadow-md ${update.type === 'ALERT' ? 'bg-amber-50 border-amber-100' : 'bg-white border-stone-100'}`}>
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${update.type === 'ALERT' ? 'bg-amber-200 text-amber-700' : 'bg-king-primary/10 text-king-primary'}`}>
                            {update.type === 'ALERT' ? '!' : '📅'}
                        </div>
                        <div>
                            <h4 className={`font-bold text-sm mb-1 ${update.type === 'ALERT' ? 'text-amber-900' : 'text-king-text'}`}>{update.title}</h4>
                            <p className="text-xs text-stone-500 leading-relaxed">{update.body}</p>
                            {update.type === 'ALERT' && (
                                <button onClick={() => onNavigate('community')} className="text-[10px] font-bold uppercase tracking-widest text-amber-700 mt-2 hover:underline">View Board</button>
                            )}
                        </div>
                    </div>
                ))}
