    ...
    <div className={`flex flex-col h-full bg-white ${embedded ? '' : 'h-[calc(100vh-80px)] md:h-screen max-w-3xl mx-auto'}`}>
      ...
      <header className={`p-4 border-b border-stone-100 flex items-center gap-4 bg-white/95 backdrop-blur sticky top-0 z-20 ${embedded ? 'justify-between' : ''}`}>
        ...
        <div className="flex items-center gap-4">
            ...
            {(!embedded || mobileMode) && (
                <button onClick={onNavigateToLibrary} className="w-8 h-8 flex items-center justify-center text-stone-400 hover:text-king-primary hover:bg-stone-50 rounded-full transition-colors active:scale-90">←</button>
            )}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 bg-king-cream/30 custom-scrollbar">
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-slide-up`}>
            <div className={`max-w-[90%] p-4 rounded-2xl leading-relaxed text-sm ${msg.role === 'user' ? 'bg-king-primary text-white rounded-tr-none shadow-glow' : 'bg-white border border-stone-100 text-king-text rounded-tl-none shadow-sm'}`}>
      ...
      <form onSubmit={handleSend} className="p-3 bg-white border-t border-stone-100 z-20 pb-safe">
          <div className="relative group">
            ...
            />
            <button type="submit" disabled={isLoading || !input.trim()} className="absolute right-3 top-3 text-king-primary font-bold hover:scale-110 active:scale-90 transition-transform disabled:opacity-50">→</button>
          </div>
      </form>
    </div>
  );
