          ...
          <div className={`flex flex-col relative h-full bg-king-cream ${className}`}>
              ...
              <div className="px-6 py-4 bg-king-cream/95 backdrop-blur z-20 border-b border-stone-100 flex items-center justify-between sticky top-0 flex-shrink-0">
                 ...
                 <div className="flex gap-2 items-center">
                     <button onClick={toggleSpeech} className={`w-10 h-10 rounded-full flex items-center justify-center transition-all active:scale-90 ${isPlaying ? 'bg-king-primary text-white shadow-glow animate-pulse' : 'bg-stone-100 text-stone-400 hover:text-king-primary'}`}>
                        {isPlaying ? (
                             <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="4" width="4" height="16" /><rect x="14" y="4" width="4" height="16" /></rect></svg>
                        ) : (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-4 mt-auto">
                            <div className="flex-1 bg-stone-100 h-1.5 rounded-full overflow-hidden">
                                <div className="bg-king-primary h-full rounded-full" style={{ width: `${(getProgress(book.id) / (book.totalChapters||1)) * 100}%` }}></div>
                            </div>
