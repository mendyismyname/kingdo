...
const getSystemInstruction = (profile: UserProfile): string => {
  ...
  return `You are "King", a digital spiritual mentor designed to bring the user closer to the Creator. User Demographic: ${demographic}. User Community: ${community || 'General'}. User Level: ${level}/10. Language Mode: ${language || 'English'}. Directives: 1. ${tone} DO NOT sound like an AI assistant. Do not say "How can I help you today?". Be direct. 2. ${terminology} 3. ${complexity} 4. Primary Quote Sources: Prioritize quotes and wisdom from ${quoteSource}. 5. The ultimate goal is Action. Always try to ground abstract concepts into a physical action or habit. 6. **FORMATTING RULE**: Return plain text only. Do not use markdown like **bold** or *italics*. `;
};
...
export const generateDailyInsight = async (profile: UserProfile): Promise<Insight> => {
  ...
  const prompt = `Generate a daily insight for me. It should include: 1. A short, profound concept (1-2 sentences) from relevant sources. 2. The source citation. 3. A single, concrete, small actionable step I can take today. Return as JSON.`;
...
export const generateNewsFeed = async (profile: UserProfile): Promise<NewsItem[]> => {
  ...
  const prompt = `Generate 4 short news headlines/summaries based on these interests: [${interests}]. For each item: 1. Create a realistic headline relevant to the interest (e.g., if 'Business', mention a market trend; if 'Hip Hop', mention culture). 2. Provide a 'spiritualInsight' that connects this news to the user's demographic/spiritual path. Return as JSON Array.`;
...
export const suggestHabitFromChat = async (profile: UserProfile, chatContext: string): Promise<any> => {
  ...
  const systemInstruction = getSystemInstruction(profile);
  const prompt = `Based on this conversation context: "${chatContext}", suggest a concise habit I can track. Return JSON: { "title": "string", "category": "THOUGHT" | "SPEECH" | "ACTION", "frequency": "DAILY" | "WEEKLY" }`;
...
export const generateBookLesson = async (profile: UserProfile, bookTitle: string, chapter: number): Promise<BookLesson> => {
  ...
  const systemInstruction = getSystemInstruction(profile);
  const prompt = `Teach me ${bookTitle}, Chapter (or Section) ${chapter}. Provide a concise lesson. Return JSON with: 1. title: Title of the concept/chapter. 2. content: The main explanation (2-3 paragraphs max). 3. summary: A one-sentence takeaway. 4. practicalApplication: A specific action or thought for the day based on this chapter. `;
