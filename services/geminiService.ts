...
const getSystemInstruction = (profile: UserProfile): string => {
  ...
  return `You are "King", a digital spiritual mentor designed to bring the user closer to the Creator.
  User Demographic: ${demographic}.
  User Community: ${community || 'General'}.
  User Level: ${level}/10.
  Language Mode: ${language || 'English'}.
...
export const generateDailyInsight = async (profile: UserProfile): Promise<Insight> => {
  ...
  const prompt = `Generate a daily insight for me. 
  It should include:
        ...
        config: {
            ...
            responseSchema: {
                ...
                properties: {
                    ...
                    category: { type: Type.STRING, enum: ["THOUGHT", "SPEECH", "ACTION"] },
