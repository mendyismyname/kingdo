...
const App: React.FC = () => {
    ...
    useEffect(() => {
        console.log('🔄 App: Initializing Supabase session check...');
        supabase.auth.getSession().then(({ data: { session } }) => {
            console.log('🔄 App: Got initial session:', session ? 'User logged in' : 'No user');
            setSession(session);
            if (session) {
        ...
        return () => {
            console.log('🧹 App: Cleaning up Supabase auth subscription');
    ...
    useEffect(() => {
        const fetchUserProfile = async () => {
            if (session) {
                console.log('🔄 App: Fetching user profile for session user ID:', session.user.id);
                try {
                    ...
                    if (error && error.code === 'PGRST116') {
                        console.log('ℹ️ App: No profile found for user, triggering onboarding');
                    ...
                    if (data) {
                        console.log('✅ App: Profile fetched successfully');
                        setProfile({
                            id: data.id,
                            name: data.name,
                        ...
                        // Load habits and book progress from local storage for now
                    ...
                    console.error('❌ App: Error fetching user profile:', error.message);
                }
            } else {
                console.log('ℹ️ App: No session, clearing profile and data');
                setProfile(null);
                setHabits([]);
                setBookProgress([]);
                setIsOnboardingNeeded(false);
    ...
    const handleOnboardingComplete = async (newProfileData: Omit<UserProfile, 'id' | 'totalStudyMinutes' | 'theme' | 'language'>) => {
        if (!session?.user) {
            console.error('❌ App: No active session to complete onboarding.');
            return;
        }
        ...
        try {
            ...
            console.error('❌ App: Error completing onboarding:', error.message);
        }
    };
    ...
    const updateTheme = async (theme: AppTheme) => {
        if (profile && session?.user) {
            try {
                ...
                console.error('❌ App: Failed to update theme: ' + error.message);
            }
        }
    };
    ...
    const updateLanguage = async (language: AppLanguage) => {
        if (profile && session?.user) {
            try {
                ...
                console.error('❌ App: Failed to update language: ' + error.message);
            }
        }
    }
    ...
    const updateCommunity = async (community: string) => {
        if (profile && session?.user) {
            try {
                ...
                console.error('❌ App: Failed to update community: ' + error.message);
            }
        }
    }
    ...
    const updateName = async (name: string) => {
        if (profile && session?.user) {
            try {
                ...
                console.error('❌ App: Failed to update name: ' + error.message);
            }
        }
    }
    ...
    const resetProfile = async () => {
        if (confirm("Reset all data? This will sign you out and delete your profile from Supabase.")) {
            try {
                ...
                console.error('❌ App: Failed to reset profile: ' + error.message);
            }
        }
    }
